...
function withSoilSensor(bytes , start, response) {
    ...
    response['soil'] = {
        'temp1': toSoilTemp(bytes[start], bytes[start + 1]),
        'hum1': toSoilMoisture(bytes[start + 2], bytes[start + 3]),
        'temp2': toSoilTemp(bytes[start + 4], bytes[start + 5]),
        'hum2': toSoilMoisture(bytes[start + 6], bytes[start + 7]),
    }
